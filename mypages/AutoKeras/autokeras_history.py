...
def plot_training_history():
    ...
    st.markdown("""
        ...
        - **Точность**: `{accuracy:.4%}` (99.9982%)
        - **Ошибка**: `{error:.2e}` (0.0076%)
    ...
    with col1:
        st.metric("Итоговая точность", f"{accuracy:.4%}", delta="0.0018% до 100%")
    with col2:
        st.metric("Финальная ошибка", f"{error:.2e}", delta="-9.24e-05")
    ...
    st.markdown("""
        ...
        - **Точность**: `{test_acc:.4%}` (99.9984%) 
        - **Ошибка**: `{test_err:.2e}` (0.0086%)
